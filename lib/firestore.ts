...
titleAr: "⭐ تقييم جديد",
    ...
    bodyAr: `تلقّيت ${data.stars} نجمة${data.comment ? ` : "${data.comment.slice(0, 40)}"` : ""}`,
...
titleAr: "👑 تم تفعيل الاشتراك!",
    ...
    bodyAr: "أصبحت الآن أستاذاً مميزاً.",
    link: "/dashboard",
    read: false,
    createdAt: new Date().toISOString(),
  });
...
titleAr: "📩 طلب تسجيل جديد",
    ...
    bodyAr: `${data.studentName} يرغب في الالتحاق بـ "${data.classeTitle}"`,
    link: `/dashboard`,
    read: false,
    createdAt: new Date().toISOString(),
  });
...
titleAr: "✅ تمت الموافقة على طلبك!",
    ...
    bodyAr: `يمكنك الآن الوصول إلى "${request.classeTitle}"`,
...
export async function updateClasseWithNotification(
  ...
  for (const enr of enrollments) {
    await createNotification({
      ...
      type: "message",
      title: "📝 Cours modifié",
titleAr: "📝 تم تعديل الدرس",
      body: `Le professeur a modifié ${changeText} du cours "${updates.title || originalClasse.title}"`,
      bodyAr: `عدّل الأستاذ ${changeText} في درس "${updates.title || originalClasse.title}"`,
...
titleAr: "❌ تم إلغاء الدرس",
      ...
      bodyAr: `أُلغي درس "${classe.title}" من طرف الأستاذ. تواصل معه لمزيد من المعلومات.`,
      link: `/mes-cours`,
      read: false,
      createdAt: new Date().toISOString(),
    });
...
titleAr: "💰 تم تسجيل الدفع",
      ...
      bodyAr: `تمّ تسجيل دفعتك البالغة ${data.amount.toLocaleString()} دج.`,
      link: "/revenus",
      read: false,
      createdAt: now.toISOString(),
    });
...
export async function sendPaymentReminder(data: {
  ...
  const bodyAr = data.isOverdue
    ? `عمولتك البالغة ${data.balance.toLocaleString()} دج في الانتظار منذ ${data.daysSince} يوماً. يرجى التسوية سريعاً.`
    : `لديك ${data.balance.toLocaleString()} دج من العمولة للتسوية. راجع بيانك في تبويب الإيرادات.`;
  ...
  for (const classeId of classeIds) {
    ...
    for (const iso of dates) {
      ...
      await addDoc(collection(db, "notifications"), {
        ...
        titleAr: kind === "1h" ? "⏰ درسك يبدأ قريباً" : "📅 درس غداً",
        body: kind === "1h"
          ? `« ${c.title} » démarre dans moins d'une heure. La salle ouvre 15 min avant.`
          : `« ${c.title} » aura lieu ${when}.`,
        bodyAr: kind === "1h"
          ? `« ${c.title} » يبدأ خلال أقل من ساعة. تفتح القاعة قبل 15 دقيقة.`
          : `« ${c.title} » سيُقام ${when}.`,
        link: `/classe/${classeId}`,
        read: false,
        createdAt: new Date().toISOString(),
...
titleAr: "📎 وثيقة جديدة متاحة",
        body: `« ${data.title} » a été ajouté au cours « ${title} ».`,
        bodyAr: `أُضيفت « ${data.title} » إلى درس « ${title} ».`,
...
titleAr: "📋 تسجيل جديد في قائمة الانتظار",
      ...
      bodyAr: `سجّل ${data.studentName} في قائمة انتظار « ${data.classeTitle} ».`,
      link: "/dashboard",
      read: false,
      createdAt: now,
    });
...
titleAr: "🎉 تحرّر مقعد!",
      ...
      bodyAr: `تحرّر مقعد في « ${e.classeTitle} ». أرسل طلبك بسرعة.`,
      link: `/classe/${classeId}`,
      read: false,
      createdAt: now,
    });
...
titleAr: "📈 ملاحظة جديدة من أستاذك",
