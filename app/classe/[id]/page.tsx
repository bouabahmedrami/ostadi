...
import { getClasseById, getEnrollmentsByStudent, getEnrollmentsByClasse, markAttendance, getRatingByStudentAndClasse, updateClasse, createNotification } from "@/lib/firestore";
import { useAuth } from "@/lib/auth-context";
import { useLang } from "@/lib/lang-context";
import { Classe, Enrollment } from "@/lib/types";
import { Video, Users, Clock, ArrowLeft, MessageCircle, CheckCircle, Star, MapPin, Calendar, Sparkles } from "lucide-react";
...
export default function ClassePage() {
  ...
  async function startLive() {
    ...
    // Notifie tous les élèves inscrits que le cours démarre
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: '#0A0014' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
        <div style={{ position: 'relative', width: '56px', height: '56px' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, border: '3px solid transparent', borderTopColor: '#FF8C00', borderRadius: '50%', animation: 'spin 0.8s cubic-bezier(0.5,0,0.5,1) infinite' }} />
        </div>
        <p style={{ color: '#a78bfa', fontSize: '13px', fontWeight: 500, letterSpacing: '0.5px' }}>Chargement du cours...</p>
      </div>
  ...
  return (
    <div style={{
      ...
      minHeight: '100vh',
      backgroundImage: `
        radial-gradient(circle at 20% 10%, rgba(124,58,237,0.12) 0%, transparent 50%),
      ...
      <div style={{
        ...
        <Link href="/" className="ostadi-back-link" style={{
          ...
          color: '#a78bfa', textDecoration: 'none', fontSize: '13.5px',
          fontWeight: 600, marginBottom: '22px', padding: '8px 14px',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <div className="ostadi-card ostadi-card-glow" style={{ padding: '26px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '14px', position: 'relative' }}>
              <span className="ostadi-badge ostadi-badge-purple">{classe.subject}</span>
              <span className="ostadi-badge ostadi-badge-blue">{classe.level}</span>
              {classe.status === "live" && (
                <span className="ostadi-badge ostadi-badge-live">
                  <span className="ostadi-live-dot" /> En direct
                </span>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
              {[
                { icon: <Clock style={{ width: '15px', height: '15px' }} />, label: `${classe.durationMinutes} min` },
                { icon: <Users style={{ width: '15px', height: '15px' }} />, label: `${classe.enrolledCount} inscrits` },
                { icon: <MapPin style={{ width: '15px', height: '15px' }} />, label: classe.wilaya },
              ].map((item, i) => (
          ...
          {canAccessRoom ? (
            <div className="ostadi-card" style={{ padding: '22px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px', flexWrap: 'wrap', gap: '12px' }}>
                ...
                {isTeacher && (
                  ...
                  >
                    {classe.status !== 'live' ? (
                      <><span className="ostadi-live-dot-sm" /> Marquer Live</>
                    ) : (
                      <>⏹ Terminer</>
                    )}
                  </button>
              ...
              <div className="ostadi-video-frame">
                <LiveKitVideoRoom
                  roomName={classe.jitsiRoom}
                  displayName={user?.displayName || (isTeacher ? 'Professeur' : 'Étudiant')}
                  isTeacher={isTeacher}
                />
              </div>
              ...
              {attended && (
                <div style={{
                  ...
                  <CheckCircle style={{ width: '16px', height: '16px' }} /> Présence confirmée — Merci !
                </div>
              )}
            </div>
          ) : (
            <div className="ostadi-card" style={{
              ...
              border: '1px solid rgba(255,140,0,0.25)', textAlign: 'center', padding: '44px 24px',
            }}>
              <div style={{
              ...
              <p style={{ color: '#FF8C00', fontWeight: 700, fontSize: '16px', marginBottom: '8px' }}>Vous n'êtes pas encore inscrit</p>
              <p style={{ color: '#a78bfa', fontSize: '13.5px', marginBottom: '22px', maxWidth: '340px', margin: '0 auto 22px', lineHeight: 1.6 }}>
                Contactez le professeur via WhatsApp pour vous inscrire et rejoindre ce cours.
              </p>
              {classe.whatsapp && (
                <a
                  href={`https://wa.me/${classe.whatsapp.replace(/\D/g, "")}?text=Bonjour, je voudrais m'inscrire à: ${classe.title}`}
            ...
            <div className="ostadi-card" style={{ padding: '20px' }}>
              ...
              {alreadyRated ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#34d399', fontSize: '13.5px' }}>
                  <CheckCircle style={{ width: '15px', height: '15px' }} /> Vous avez déjà évalué ce cours
                </div>
              ) : (
                <button onClick={() => setShowRating(true)} className="ostadi-btn-orange" style={{ fontSize: '13px', padding: '9px 18px' }}>
                  <Star style={{ width: '14px', height: '14px' }} /> Évaluer maintenant
                </button>
              )}
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div className="ostadi-card ostadi-card-hover" style={{ padding: '18px' }}>
              <h3 style={{ color: '#8b7bb8', fontSize: '11.5px', fontWeight: 700, margin: '0 0 14px', textTransform: 'uppercase', letterSpacing: '0.8px' }}>Professeur</h3>
            ...
            <div className="ostadi-card ostadi-card-hover" style={{ padding: '18px' }}>
              <h3 style={{ color: '#8b7bb8', fontSize: '11.5px', fontWeight: 700, margin: '0 0 14px', textTransform: 'uppercase', letterSpacing: '0.8px' }}>Détails</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '13px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ color: '#a78bfa', fontSize: '12.5px' }}>DA / {classe.priceType === "session" ? "séance" : "mois"}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <style jsx global>{`
        ...
        .ostadi-card-glow {
          box-shadow: 0 8px 32px rgba(124,58,237,0.08), inset 0 1px 0 rgba(255,255,255,0.03);
        }
        .ostadi-card-hover:hover {
          border-color: rgba(168,85,247,0.35);
          transform: translateY(-2px);
          box-shadow: 0 12px 24px rgba(124,58,237,0.12);
        }
        .ostadi-back-link:hover {
        ...
        .ostadi-badge-purple {
          background: rgba(124,58,237,0.18); color: #d8b4fe;
        ...
        .ostadi-btn-orange:hover {
          transform: translateY(-2px) scale(1.02);
          box-shadow: 0 8px 24px rgba(255,140,0,0.45);
        }
        .ostadi-btn-orange:active { transform: translateY(0) scale(0.98); }
        .ostadi-btn-red {
          display: inline-flex; align-items: center; gap: 7px;
        ...
        .ostadi-btn-red:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(239,68,68,0.4); }
        .ostadi-btn-outline {
        ...
        .ostadi-btn-outline:hover {
          background: rgba(255,140,0,0.12); border-color: rgba(255,140,0,0.6);
