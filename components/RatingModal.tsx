...
export default function RatingModal({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // ⚠️ AVANT : try/finally sans catch — en cas d'échec le modal
      // restait ouvert sans message, l'élève cliquait plusieurs fois
      console.error("Envoi de l'avis échoué :", err);
      setError(
        err?.code === "permission-denied"
          ? (isRTL ? "لا يمكنك تقييم هذا الدرس." : "Vous ne pouvez pas évaluer ce cours.")
          : (isRTL ? "فشل الإرسال. تحقق من اتصالك." : "Échec de l'envoi. Vérifiez votre connexion.")
      );
      setLoading(false);
    }
  }
